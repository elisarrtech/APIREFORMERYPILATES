...
/**
 * Principles - sección de 6 principios (flow, centring, breath, concentration, control, precisión)
 * Uso: <Principles />
 */
export default function Principles() {
  const items = [
    ...
    { key: "centring", title: "centring", svg: (
      <svg viewBox="0 0 24 24" className="w-10 h-10" aria-hidden>
    ...
    { key: "breath", title: "breath", svg: (
      <svg viewBox="0 0 24 24" className="w-10 h-10" aria-hidden>
        <path d="M12 3v18M8 6c1.5 2 2 2 4 2s2.5 0 4-2" stroke="currentColor" strokeWidth="1.8" fill="none" strokeLinecap="round"/>
      </svg>
    )},
    { key: "concentration", title: "concentration", svg: (
      <svg viewBox="0 0 24 24" className="w-10 h-10" aria-hidden>
        <circle cx="12" cy="12" r="8" stroke="currentColor" strokeWidth="1.8" fill="none"/>
    ...
    { key: "control", title: "control", svg: (
      <svg viewBox="0 0 24 24" className="w-10 h-10" aria-hidden>
        <path d="M12 4v16M8 12c2-2 4-2 8 0" stroke="currentColor" strokeWidth="1.8" fill="none" strokeLinecap="round"/>
      </svg>
    )},
    { key: "precision", title: "precisión", svg: (
      <svg viewBox="0 0 24 24" className="w-10 h-10" aria-hidden>
        <circle cx="12" cy="12" r="8" stroke="currentColor" strokeWidth="1.8" fill="none"/>
        <path d="M6 12c2 0 2-2 6-2s4 2 6 2" stroke="currentColor" strokeWidth="1.8" fill="none" strokeLinecap="round"/>
      </svg>
    )},
  ];
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 max-w-6xl mx-auto items-center">
          {items.map((it) => (
            <div key={it.key} className="flex flex-col items-center text-center">
              <div className="p-3 rounded-full border border-sage-300 w-16 h-16 flex items-center justify-center text-sage-700 principle-icon">
                {it.svg}
              </div>
              <div className="mt-4">
                <div className="principle-title">{it.title}</div>
              </div>
            </div>
          ))}
        </div>
